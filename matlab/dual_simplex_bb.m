function [obj,x,B,status] = dual_simplex_bb(c,A,b,eps,x0,B)

% Solves:- Max c^Tx subject to Ax = b, x >= 0
% using the dual simplex method
% We are also given an initial dual feasible basis B and the basic solution
% x0
% [obj,x,B,status] = dual_simplex_bb(c,A,b,eps,x0)
% eps is a suitable tolerance, say 1e-6
% The method implements the dual simplex method in Box 10.1 on page 155
% of Chvatal
% status = 1 if the solution is optimal
% status = 2 if the LP is infeasible
% The output B contains the optimal basis for the LP

% Kartik's MATLAB code for MA/OR/IE 505
% February 20, 2006
% Last modified: April 19th, 2006
% Designed for use with the branch and bound scheme MILP_bb.m
